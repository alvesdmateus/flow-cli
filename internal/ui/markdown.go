...
func renderCodeBlock(code, lang string) string {
	...
	var header string
	if lang != "" {
		header = dimStyle.Render("  " + lang) + "\n"
	}

	// Syntax highlighting based on language (basic)
	highlighted := highlightSyntax(code, lang)

	return header + mdCodeBlockStyle.Render(highlighted)
}

// highlightSyntax provides basic syntax highlighting
func highlightSyntax(code, lang string) string {
	if lang == "" {
		return code
	}
...
func highlightLine(line, lang string) string {
	...
	numberStyle := lipgloss.NewStyle().Foreground(lipgloss.Color("209"))
	...
	if prefix, ok := commentPrefixes[lang]; ok {
		if idx := strings.Index(line, prefix); idx >= 0 {
			before := line[:idx]
			comment := line[idx:]
			return highlightLine(before, lang) + commentStyle.Render(comment)
		}
	}

	// Keywords by language
	keywords := map[string][]string{
		"go":         {"func", "package", "import", "var", "const", "type", "struct", "interface", "return", "if", "else", "for", "range", "switch", "case", "default", "defer", "go", "select", "chan", "map", "make", "new", "nil", "true", "false"},
		"javascript": {"function", "const", "let", "var", "return", "if", "else", "for", "while", "switch", "case", "default", "class", "extends", "import", "export", "from", "async", "await", "try", "catch", "throw", "new", "this", "null", "undefined", "true", "false"},
		"typescript": {"function", "const", "let", "var", "return", "if", "else", "for", "while", "switch", "case", "default", "class", "extends", "import", "export", "from", "async", "await", "try", "catch", "throw", "new", "this", "null", "undefined", "true", "false", "interface", "type", "enum", "implements"},
		"python":     {"def", "class", "return", "if", "elif", "else", "for", "while", "try", "except", "finally", "with", "as", "import", "from", "pass", "break", "continue", "and", "or", "not", "in", "is", "None", "True", "False", "self", "lambda", "yield", "async", "await"},
		"rust":       {"fn", "let", "mut", "const", "struct", "enum", "impl", "trait", "pub", "use", "mod", "if", "else", "match", "for", "while", "loop", "return", "break", "continue", "self", "Self", "true", "false", "Some", "None", "Ok", "Err"},
	}

	result := line

	// Highlight strings (basic - single and double quotes)
	...
	if kws, ok := keywords[lang]; ok {
		for _, kw := range kws {
			...
			result = kwRegex.ReplaceAllStringFunc(result, func(match string) string {
